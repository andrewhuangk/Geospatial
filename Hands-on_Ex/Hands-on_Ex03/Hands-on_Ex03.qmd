...
Worth checking for duplicate points before going further — two preschools can easily sit at the exact same coordinate if they're in the same building, and some of the tests coming up assume unique locations.
...
Two things worth being upfront about: this uses Ex01's preschool/subzone data instead of the childcare-centre dataset the textbook chapter actually works with, and the study boundary is a dissolved subzone outline standing in for a coastline shapefile that isn't available here. Not the original dataset, and better to say so than pretend otherwise.

One thing worth trying later if there's time: running this on a single planning area instead of the whole island. Testing CSR against all of Singapore at once is a pretty blunt instrument — preschools are almost certainly clustering because that's where people actually live, not because of anything special about land area on its own.
